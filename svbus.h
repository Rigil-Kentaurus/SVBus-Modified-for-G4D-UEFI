...
typedef struct _DEVICE_EXTENSION
{
	...
	union
	{
		...
		struct
		{
			...
			ULONGLONG ImageFragment_address;        //碎片地址
...
typedef struct _GRUB4DOS_DRIVE_MAP_SLOT
{
	...
	USHORT :10;              // bit 0-9: max cylinder of the TO drive
	USHORT fragment:1;       // bit 10:      碎片
	USHORT :2;               // bit 11-12:
...
typedef struct _GRUB4DOS_FRAGMENT_MAP_SLOT
{
	USHORT slot_len;
	UCHAR from;
	UCHAR to;
	ULONGLONG fragment_data[0];
}GRUB4DOS_FRAGMENT_MAP_SLOT,*PGRUB4DOS_FRAGMENT_MAP_SLOT;
...
// pool tag for the allocated memory  分配内存的池标记 
