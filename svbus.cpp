...
/*
	...
	Copyright (C) 2020 Kai Schtrom , Modified By Sunsea@wuyou
...
//插入碎片计算
          ULONGLONG fragment_len = 0, total = 0;
          ULONG j = 0, read_len, LengthBackups = Length;
          PFRAGMENT data = (PFRAGMENT)DeviceExtension->Disk.ImageFragment_address;
          if (DeviceExtension->Disk.ImageFragment_address)  //有碎片
          {
			  KdPrint(("Do fragment calc\n"));
            //如果是光盘, StartSector*4
            StartSector =  (StartSector * DeviceExtension->Disk.SectorSize) >> 9;
            //确定Form扇区起始在哪个碎片
            while (1)
            {
              total += data[j++].sector_count;
              if (StartSector < total)		//确定起始位置的条件
                break;
            }
            //确定本碎片最大访问字节
            fragment_len = (total - StartSector) << 9;
            //确定Form扇区起始的确切位置(To_start(j)+偏移)
            StartSector += data[j-1].start_sector + data[j-1].sector_count - total;
          ...
          {
            //判断本碎片可否一次访问完毕
            if (LengthBackups > fragment_len)
              read_len = fragment_len;  
            else
              read_len = LengthBackups;
            
            // 0x28 READ (10) or 0x88 READ (16)  读(10)或(16)
...
//						tmpBuffer = ExAllocatePoolWithTag(PagedPool,(SIZE_T)Length,SVBUS_POOL_TAG);
              tmpBuffer = ExAllocatePoolWithTag(PagedPool,(SIZE_T)read_len,SVBUS_POOL_TAG);
              if(tmpBuffer == NULL)
              {
                Srb->DataTransferLength = 0;
...
//            Status = ReadWritePhysicalDisk(DeviceExtension->Disk.FileDeviceObject,DeviceExtension->Disk.FileObject,IRP_MJ_READ,tmpBuffer,Length,&offset);
              Status = ReadWritePhysicalDisk(DeviceExtension->Disk.FileDeviceObject,DeviceExtension->Disk.FileObject,IRP_MJ_READ,tmpBuffer,read_len,&offset);
						if(!NT_SUCCESS(Status))
						{
...
//						RtlCopyMemory(buffer,tmpBuffer,(SIZE_T)Length);
						RtlCopyMemory(buffer,tmpBuffer,(SIZE_T)read_len);
...
//						Status = ReadWritePhysicalDisk(DeviceExtension->Disk.FileDeviceObject,DeviceExtension->Disk.FileObject,IRP_MJ_WRITE,buffer,Length,&offset);
						Status = ReadWritePhysicalDisk(DeviceExtension->Disk.FileDeviceObject,DeviceExtension->Disk.FileObject,IRP_MJ_WRITE,buffer,read_len,&offset);
						if(!NT_SUCCESS(Status))
						{
							Srb->DataTransferLength = 0;
							Srb->SrbStatus = SRB_STATUS_ERROR;
            ...
            //判断是否需要再读
            LengthBackups -= read_len;
            if (LengthBackups)
            {
              //确定下一碎片扇区起始及尺寸
              StartSector = data[j].start_sector;
              offset.QuadPart = (LONGLONG)(StartSector * 512);
              fragment_len = data[j++].sector_count << 9;
			  char* p_temp = (char*)buffer; //sunsea注：M$编译器是个狗屎烂蛋，此处必须用此种奇技淫巧才能对指针进行加减，选用char*是因为：1,确保1字节。加减正确；2，自动适应x64和x86情况。
              p_temp += read_len;
			  buffer = p_temp;
            }
          }//while (LengthBackups)
...
NTSTATUS BusAddChild(IN PDEVICE_OBJECT BusDeviceObject,IN ULONG i)
{
  ...
  //碎片
  if(DriverExtension->slot[i].fragment == 1)
  {
    PGRUB4DOS_FRAGMENT_MAP_SLOT q = (PGRUB4DOS_FRAGMENT_MAP_SLOT)&DriverExtension->chip[0];
    ULONG n = 0x280;
    ...
    while (n <= 0x280)
    {
      if (!q->slot_len)
        break;
      if (q->from == DriverExtension->slot[i].from_drive)
      {
        DeviceExtension->Disk.ImageFragment_address = (ULONGLONG)((UCHAR *)q + 4);
        break;
      }
      n -= q->slot_len;
      q = (PGRUB4DOS_FRAGMENT_MAP_SLOT)((UCHAR *)q + q->slot_len);
    }
    if (n > 0x280)
      DeviceExtension->Disk.ImageFragment_address = 0;
  }
...
//	DeviceExtension->Disk.DiskGeometry.TracksPerCylinder = DriverExtension->slot[i].max_head + 1;
  DeviceExtension->Disk.DiskGeometry.TracksPerCylinder = 0xff;  //磁头数
//	DeviceExtension->Disk.DiskGeometry.SectorsPerTrack = DriverExtension->slot[i].to_sector;
  DeviceExtension->Disk.DiskGeometry.SectorsPerTrack = 0x3f;    //to扇区数
...
NTSTATUS BusAddDevice(IN PDRIVER_OBJECT DriverObject,IN PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	KdPrint(("I'm here:BusAddChild routine\n."));
	...
	mapSize = intVector.Offset + ((SIZE_T)0x400);
	// map physical address range to nonpaged system space
	// we should map this non cached, otherwise Driver Verifier shows the following error in WinDbg: Iospace mapping overlap
	// the reason is because the OS has already mapped the same address range with another cache type
	virtAddr = (PUCHAR)MmMapIoSpace(physAddr,mapSize,MmNonCached);
	...
	KdPrint(("I'm here:Check $INT13SFGRUB4DOS!\n"));
...
//				RtlCopyMemory(&DriverExtension->slot[0],&virtAddr[k - 0xE3],sizeof(GRUB4DOS_DRIVE_MAP_SLOT) * MAXIMUM_GRUB4DOS_DRIVE_MAP_SLOTS);
				KdPrint(("Checked! $INT13SFGRUB4DOS k+0x103 @ 0x%X,Table start @ 0x%X\n", i+ k + 0x103,i+k+0x20));
        RtlCopyMemory(&DriverExtension->slot[0],&virtAddr[k + 0x20],sizeof(GRUB4DOS_DRIVE_MAP_SLOT) * MAXIMUM_GRUB4DOS_DRIVE_MAP_SLOTS);
        j = 1;
      }
      else if(RtlCompareMemory(&virtAddr[k + 0xe3],"$INT13SFGRUB4DOS",(SIZE_T)0x10) == 0x10)
      {
				KdPrint(("Checked! $INT13SFGRUB4DOS k+0xe3 @ 0x%X,Table start @ 0x%X\n", i + k + 0xe3, i + k ));
        RtlCopyMemory(&DriverExtension->slot[0],&virtAddr[k],sizeof(GRUB4DOS_DRIVE_MAP_SLOT) * MAXIMUM_GRUB4DOS_DRIVE_MAP_SLOTS);
        j = 1;
      }
        
      //新加  检查碎片标识字符串"FRAGMENT"
      if(j == 1)
      {
		  if (RtlCompareMemory(&virtAddr[k + 0x140], "FRAGMENT", (SIZE_T)8) == 8) {
			  RtlCopyMemory(&DriverExtension->chip[0], &virtAddr[k + 0x148], 0x280);
			  KdPrint(("Checked Fragments @ 0x%X!\n",i+k+0x140));
		  }
	...
	KdPrint(("Not Checked $INT13SFGRUB4DOS!\n"));
	return STATUS_SUCCESS;
}
